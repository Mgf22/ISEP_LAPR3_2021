...
public class ContainersStore {
     ...
     * @return returns string with a list of all unloaded containers in the next port
     */
    public String getContainers(int mmsi){
        ...
        try{
            ...
            String sql = "SELECT f.name name, m.id_manifest_lu manifest " +
                    ...
                    "WHERE m.id_type_manifest = tm.id_type_manifest AND tm.type_manifest = 'Unload' AND m.date_manifest > CURRENT_DATE AND f.id_facility = m.id_facility AND mt.id_manifest_travel = m.id_manifest_travel AND mt.id_vehicle = v.id_vehicle AND v.id_vehicle = s.id_vehicle AND s.id_mmsi = '" + mmsi +
