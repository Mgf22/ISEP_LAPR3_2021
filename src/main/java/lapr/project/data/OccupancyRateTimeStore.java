 ...
 * @author Jorge Lima
 */
public class OccupancyRateTimeStore {
    /**
     ...
     */
    public double getOccupancyRate(int mmsi, String date){
        double ret = 0;
        try{
            ...
            String sql = "SELECT (COUNT(id_container_manifest)/avg(capacity))*100 cont " +
                    ...
                    "WHERE cm.id_manifest_lu = m.id_manifest_lu AND m.id_manifest_travel = mt.id_manifest_travel AND mt.id_vehicle = v.id_vehicle AND v.id_vehicle = s.id_vehicle AND s.id_mmsi = '" + mmsi + "' AND mt.date_manifest = TO_DATE('" + date + "','YY-MM-DD')";
